...
@click.option('-r', '--rules_file', help='The YARA rules file.', default="yara_rules.yar")
def main(db_file, user, channel, rules_file):
    threads = get_threads(db_file, channel_id=channel, user_id=user)
    yara_match = analyze_yara(threads, user_id=user, channel_id=channel, rules_file=rules_file, verbose=True)
